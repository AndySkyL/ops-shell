...
echo "`date +'%Y-%m-%d:%H:%M:%S'`|$pid|state:[slaver] Run 'SLAVEOF 192.168.56.11 6379'" >> $LOGFILE
$REDISCLI SLAVEOF 192.168.56.11 6379 >> $LOGFILE  2>&1
echo "`date +'%Y-%m-%d:%H:%M:%S'`|$pid|state:[slaver] slave connect to 192.168.56.11 ok..." >> $LOGFILE
