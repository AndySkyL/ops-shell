...
for ((i=1; i < ${#array[*]}; i++));do
    if [[ ${array[$i]} == 'wtp-service-security' ]];then
        stopbootapp  -a wtp-service-security && nohup java -jar `ls |grep wtp-service-security*.jar` --spring.profiles.active=prod --auth-server=http://119.253.63.16/security >/dev/null 2>&1&
    elif [[ ${array[$i]} == 'wtp-eureka-server' ]];then
        stopbootapp  -a wtp-eureka-server &&  startbootapp -a wtp-eureka-server -p prod1 >> /tmp/wtp-server.log
    else 
        stopbootapp  -a ${array[$i]} && startbootapp -p prod -a ${array[$i]} >> /tmp/wtp-server.log
